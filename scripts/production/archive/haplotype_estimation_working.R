...
# Working Haplotype Estimation Functions
# Extracted from the 49-hour-running production code
# This contains the exact functions that are currently running successfully

library(tidyverse)
library(limSolve)

#' Estimate Haplotype Frequencies (Working Version)
#' 
#' This is the exact function that's been running successfully for 49+ hours
#' 
...
estimate_haplotypes_list_format <- function(pos, sample_name, df3, founders, h_cutoff,
  ...
  # Return the 4 required fields (matching the working function)
  return(list(Groups=groups, Haps=founder_frequencies, Err=if (!is.null(final_result)) final_result$cov else matrix(NA, length(founders), length(founders)), Names=founders))
}

#' Process RefAlt data into df3 format (from working code)
process_refalt_data <- function(refalt_file, founders) {
  ...
  # Transform data (same as working code)
  df2 <- df %>%
  ...
  # Apply quality filter (same as working code)
  founder_wide <- df2 %>%
