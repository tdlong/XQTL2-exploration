...
# Simple benchmark using EXACT production functions
# No modifications, no "fixes" - just copy and run

library(tidyverse)
library(limSolve)
library(MASS)

# Source the EXACT production functions
source("scripts/ErrMatrix/production_functions_exact.R")

# Parse command line arguments
args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 3) {
  stop("Usage: Rscript benchmark_exact.R <chr> <output_dir> <param_file>")
}
...
cat("=== BENCHMARKING EXACT PRODUCTION FUNCTIONS ===\n")
cat("Chromosome:", chr, "\n")
...
# Load RefAlt data using EXACT production function
...
# Save results
output_file <- file.path(output_dir, paste0("exact_baseline_", chr, ".RDS"))
