...
est_haps_var <- function(testing_position, sample_name, df3, founders, h_cutoff,
                               ...
                               chr = "chr2R",
                               verbose = 0) {
  # [Full function content - same as in debug_wrapper.R]
  # ... (I'll include the full function here)
}
...
for (h_cutoff in h_cutoff_values) {
  ...
  result <- do.call(est_haps_var, c(
    list(df3 = hunk_data$df3), 
    hunk_data$args,
    list(h_cutoff = h_cutoff, verbose = 1)
  ))
