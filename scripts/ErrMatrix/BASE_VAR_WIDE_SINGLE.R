...
#
# Usage:
#   Rscript scripts/ErrMatrix/BASE_VAR_WIDE_SINGLE.R <chr> <position> <sample_name> <output_dir> <param_file>
...
if (length(args) != 5) {
  cat("Usage: Rscript scripts/ErrMatrix/BASE_VAR_WIDE_SINGLE.R <chr> <position> <sample_name> <output_dir> <param_file>\n")
  quit(status = 1)
}
...
cat("Founders:", paste(founders, collapse=","), " h_cutoff:", h_cutoff, "\n\n")
