...
# Use production code directly - no copying, no "fixing"
# Just source the working production file and skip smoothing
...
if (length(args) < 3) {
  stop("Usage: Rscript benchmark_production_direct.R <chr> <output_dir> <param_file>")
}
...
cat("=== BENCHMARKING PRODUCTION CODE DIRECTLY ===\n")
cat("Chromosome:", chr, "\n")
...
# Load RefAlt data using production function directly
...
# Load data using production function
