...
# REFALT2haps Adaptive Window - Haplotype Frequency Estimation
# This script estimates haplotype frequencies using adaptive window expansion
# and outputs both frequency estimates AND distinguishability quality flag
# 
# Usage: Rscript REFALT2haps.AdaptWindow.Single.R <chr> <parfile> <mydir> <h_cutoff> <euchrom_start> <euchrom_end>
# Example: Rscript REFALT2haps.AdaptWindow.Single.R chr2R helpfiles/JUICE_haplotype_parameters.R process/test 6 5398184 24684540
...
if (length(args) != 6) {
  cat("Usage: Rscript REFALT2haps.AdaptWindow.Single.R <chr> <parfile> <mydir> <h_cutoff> <euchrom_start> <euchrom_end>\n")
  cat("Example: Rscript REFALT2haps.AdaptWindow.Single.R chr2R helpfiles/JUICE_haplotype_parameters.R process/test 6 5398184 24684540\n")
  quit(status = 1)
}
...
parfile <- args[2]
mydir <- args[3]
h_cutoff_used <- as.numeric(args[4])
euchrom_start <- as.numeric(args[5])
euchrom_end <- as.numeric(args[6])

cat("=== Adaptive Window Haplotype Estimation ===\n")
cat("Chromosome:", mychr, "\n")
cat("H_cutoff:", h_cutoff_used, "\n")
...
cat("Running adaptive window haplotype estimation...\n")
...
if (nrow(results_df) > 0) {
  ...
  # Save to RDS file
  output_file <- file.path(mydir, paste0("adaptive_window_h", h_cutoff_used, "_results_", mychr, ".RDS"))
