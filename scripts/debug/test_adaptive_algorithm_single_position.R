...
# =============================================================================
# 
# This script tests the adaptive algorithm on a single position to see what's
# happening and why it might be failing to converge.
#
# USAGE:
# Rscript scripts/debug/test_adaptive_algorithm_single_position.R <chr> <param_file> <output_dir> <position>
#
# EXAMPLE:
# Rscript scripts/debug/test_adaptive_algorithm_single_position.R chr2R helpfiles/ZINC2_haplotype_parameters.R process/ZINC2 5400000
# =============================================================================

# Parse command line arguments
args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 4) {
  stop("Usage: Rscript scripts/debug/test_adaptive_algorithm_single_position.R <chr> <param_file> <output_dir> <position>")
}
...
# Get data for test position ± 100kb
window_size <- 100000
...
if (nrow(window_data) == 0) {
  stop("No SNPs found in window around position", test_position)
}
...
if (!all(c(founders, test_sample) %in% names(wide_data))) {
  stop("Missing founders or sample in wide data")
}
...
if (nrow(founder_matrix_clean) < 10) {
  stop("Insufficient SNPs after cleaning:", nrow(founder_matrix_clean))
}
...
cat("Unique groups:", paste(sort(unique(groups)), collapse = ", "), "\n")
...
for (cluster_id in unique_clusters) {
  ...
  if (length(cluster_founders) == 1) {
    cat("Group", cluster_id, ":", cluster_founders, "(individual)\n")
  } else {
...
cat("Constraint matrix E dimensions:", nrow(E), "x", ncol(E), "\n")
cat("Constraint vector F length:", length(F), "\n")

# Run LSEI with fulloutput=TRUE
cat("Running LSEI...\n")
...
if (result$IsError == 0) {
  ...
  for (i in 1:length(hap_freqs)) {
    cat("  ", founders[i], ":", round(hap_freqs[i], 4), "\n")
  }
  cat("Sum:", round(sum(hap_freqs), 4), "\n")
  
  if (!is.null(result$cov)) {
    cat("Error matrix dimensions:", nrow(result$cov), "x", ncol(result$cov), "\n")
    cat("Error matrix has values:", !all(is.na(result$cov)), "\n")
  } else {
    cat("Error matrix is NULL\n")
  }
} else {
