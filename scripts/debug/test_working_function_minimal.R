...
# Test the working function and show how to capture groups and error matrices
# This will call the actual working function and show what we can capture
...
if (length(args) != 4) {
  stop("Usage: Rscript scripts/debug/test_working_function_minimal.R <chr> <param_file> <output_dir> <position>")
}
...
cat("=== TESTING WORKING FUNCTION AND CAPTURING GROUPS/ERROR MATRICES ===\n")
cat("Chromosome:", chr, "\n")
...
# Test the working function on the first sample
...
for (founder in founders) {
  cat(sprintf("  %s: %.3f\n", founder, working_result[[founder]]))
}

# Now show what we need to capture for the list format
cat("\n=== WHAT WE NEED TO CAPTURE FOR LIST FORMAT ===\n")
cat("We need to modify the working function to capture:\n")
cat("1. Groups from clustering step (cutree result)\n")
cat("2. Error matrix from lsei call (with fulloutput=TRUE)\n")
