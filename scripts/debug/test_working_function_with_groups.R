...
# Test the working function but capture groups and error matrices
# This will show us what we can capture from the working function
...
if (length(args) != 4) {
  stop("Usage: Rscript scripts/debug/test_working_function_with_groups.R <chr> <param_file> <output_dir> <position>")
}
...
cat("=== TESTING WORKING FUNCTION WITH GROUPS AND ERROR MATRICES ===\n")
cat("Chromosome:", chr, "\n")
...
# Modified version of the working function that captures groups and error matrices
estimate_haplotypes_with_groups <- function(pos, sample_name, df3, founders, h_cutoff, chr, verbose = 0) {
  ...
  # ADAPTIVE WINDOW METHOD - copied from working code but with groups and error matrix capture
...
cat("=== TESTING MODIFIED FUNCTION WITH GROUPS AND ERROR MATRICES ===\n")
cat("Test position:", test_position, "\n")
cat("Test sample:", test_sample, "\n\n")

# Call the modified function
modified_result <- estimate_haplotypes_with_groups(test_position, test_sample, observed_euchromatic, 
...
print(modified_result$haplotype_freqs)
cat("Groups:\n")
