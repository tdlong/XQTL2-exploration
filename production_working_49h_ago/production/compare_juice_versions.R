...
# Compare JUICE slow vs fast versions
# Usage: Rscript compare_juice_versions.R <chr> <n_positions> <n_samples>
# Example: Rscript compare_juice_versions.R chr2R 50 3
...
if (length(args) != 3) {
  cat("Usage: Rscript compare_juice_versions.R <chr> <n_positions> <n_samples>\n")
  cat("  chr: chromosome (e.g., chr2R)\n")
  cat("  n_positions: number of positions tested (e.g., 50)\n")
  cat("  n_samples: number of samples tested (e.g., 3)\n")
  quit(status = 1)
}
...
cat("=== COMPARING JUICE SLOW vs FAST VERSIONS ===\n")
cat("Chromosome:", chr, "\n")
cat("Positions:", n_positions, "\n")
cat("Samples:", n_samples, "\n\n")

# Load results from both versions
slow_file <- paste0("test_juice_slow_", chr, "_", n_positions, "pos_", n_samples, "samp.RDS")
fast_file <- paste0("test_juice_fast_", chr, "_", n_positions, "pos_", n_samples, "samp.RDS")
